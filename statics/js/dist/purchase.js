var curRow, curCol, loading, SYSTEM = system = parent.SYSTEM,
	...
	THISPAGE = {
		init: function(a) {
			"150502" == urlParam.transType ? this.mod_PageConfig = Public.mod_PageConfig.init("purchaseBack1") : this.mod_PageConfig = Public.mod_PageConfig.init("purchase1"), 
			...
			this.$_date.datepicker({
				onSelect: function(a) {
					if (!(originalData.id > 0)) {
						...
						b.$_number.text(""), Public.ajaxPost(defaultPage.WDURL  + "/basedata/systemProfile/generateDocNo?action=generateDocNo", {
							billType: "PUR",
							billDate: c
						}, function(a) {
...
//			}
		...
		loadGrid: function(a) {
			...
			function v(a) {
				...
				if (b) {
					b.batch || $("#grid").jqGrid("setCell", a, "batch", "&#160;"), 
					b.safeDays || ($("#grid").jqGrid("setCell", a, "prodDate", "&#160;"), 
					$("#grid").jqGrid("setCell", a, "safeDays", "&#160;"), 
					$("#grid").jqGrid("setCell", a, "validDate", "&#160;")), 
					1 == b.isWarranty && $("#grid").jqGrid("showCol", "batch"), 
					b.safeDays > 0 && ($("#grid").jqGrid("showCol", "prodDate"), 
					$("#grid").jqGrid("showCol", "safeDays"), $("#grid").jqGrid("showCol", "validDate"));
					var c = {
						skuName: b.skuName || "",
					...
					if (SYSTEM.ISSERNUM && 1 == b.isSerNum && (c.preQty = c.serNumList ? c.serNumList.length : 0), c.preQty > 0) {
						var d = parseFloat(c.preQty),
					...
					c.amount = c.amount ? c.amount : c.price * c.preQty;
					...
					if (THISPAGE.$_barCodeInsert.hasClass("active")) {
						var a = function(a) {
							var b = $("#" + a),
								c = b.next(),
								d = b.index() + 1;
							return 0 == c.length ? ($("#grid").jqGrid("addRowData", THISPAGE.newId, {}, "last"), THISPAGE.newId++, 
			...
			var C = "grid";
			w.mod_PageConfig.gridReg(C, B);
			B = w.mod_PageConfig.conf.grids[C].colModel;
			$("#grid").jqGrid({
				data: a.entries,
				datatype: "clientSide",
				autowidth: true,
				...
				afterEditCell: function(a, b, c, d, e) {
					function f() {
						...
						if (b) {
							...
							b = $.extend(!0, {}, b), b.skuName = c.skuName, b.mainUnit = c.mainUnit, b.unitId = c.unitId, b.preQty = c.preQty, b.qty = c.qty,b.price = c.price, b.discountRate = c.discountRate, b.deduction = c.deduction, b.amount = c.amount, b.taxRate = c.taxRate, b.tax = c.tax, b.taxAmount = c.taxAmount, b.locationName = c.locationName, $("#" + a).data("goodsInfo", b)
						}
					}
						...
						if (d) {
							...
							if (f || (f = {}, a.$_customer.data("priceList", f)), e && "" !== $.trim(a.$_customer.find("input").val())) {
								var g = function() {
										...
										f[d.id] = a, Public.ajaxPost(defaultPage.WDURL  + "/basedata/inventory/listBySelected?action=listBySelected", {
											type: "sa",
											ids: d.id,
											contactId: e.id
										}, function(c) {
		...
		disableEdit: function() {
			...
			this.$_note.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_discountRate.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_deduction.attr("disabled", "disabled").addClass("ui-input-dis"), this.$_payment.attr("disabled", "disabled").addClass("ui-input-dis"), this.accountCombo.disable()
//				, $("#grid").jqGrid("setGridParam", {
//				cellEdit: !1
//			}), 
				this.editable = !1
		...
		addEvent: function() {
			...
			this.$_payment.keyup(function() {
				...
				d && ("edit" === originalData.stata && (d.id = originalData.id, d.stata = "edit"), c.ajaxPost(defaultPage.WDURL  + "/scm/invPu/add?action=add", {
			...
			$(".wrapper").on("click", "#savaAndAdd", function(b) {
				...
				var c = $(this),
					d = THISPAGE.getPostData();
				d && c.ajaxPost(defaultPage.WDURL  + "/scm/invPu/addNew?action=addNew", {
					postData: JSON.stringify(d)
				}, function(b) {
					if (200 === b.status) {
			...
			$(".wrapper").on("click", "#edit", function(b) {
				if (b.preventDefault(), Business.verifyRight("PU_UPDATE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
					d && c.ajaxPost(defaultPage.WDURL + "/scm/invPu/updateInvPu?action=updateInvPu", {
						postData: JSON.stringify(d)
					}, function(b) {
			...
			$(".wrapper").on("click", "#sure", function(b) {
				if (b.preventDefault(), Business.verifyRight("PU_SURE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
						d.pu_sure=1;
					    d.surealled=0;
			...
			$(".wrapper").on("click", "#sureall", function(b) {
				if (b.preventDefault(), Business.verifyRight("PU_SUREALL")) {
					var c = $(this),
						d = THISPAGE.getPostData();
						d.pu_sure=1;
					   d.surealled=1;
			...
			$(".wrapper").on("click", "#reAudit", function(b) {
				if (b.preventDefault(), Business.verifyRight("PU_UNCHECK")) {
					var c = $(this);
					c.ajaxPost(defaultPage.WDURL + "/scm/invPu/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
						id: originalData.id
					}, function(b) {
			...
			$("#prev").click(function(b) {
				...
				Public.ajaxGet(defaultPage.WDURL  + "/scm/invPu/update?action=update", {
			...
			$("#next").click(function(b) {
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已经没有下一张了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("数据加载中...", 1e3, "loading.gif", !0), Public.ajaxGet(defaultPage.WDURL  + "/scm/invPu/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
		...
		calTotal: function() {
			for (var a = $("#grid").jqGrid("getDataIDs"), b = 0, c = 0, d = 0, e = 0, f = 0, g = 0, h = a.length; h > g; g++) {
				...
				j.preQty && (b += parseFloat(j.preQty)), j.deduction && (c += parseFloat(j.deduction)), j.amount && (d += parseFloat(j.amount)), j.tax && (e += parseFloat(j.tax)), j.taxAmount && (f += parseFloat(j.taxAmount))
			}
					...
					if (i) {
						...
						if (SYSTEM.ISSERNUM) {
							...
							if (m && m.length == Number(h.preQty));
								...
								}), $("#grid").jqGrid("editCellByColName", g, "preQty"), !1
							}
						}
						f = {
							invId: i.id,
			...
			if (f.length > 0) {
				var g = $.trim(b.$_note.val()),
					h = {
						...
						transType: originalData.transType,
						entries: f,
						totalQty: $("#grid").jqGrid("footerData", "get").preQty.replace(/,/g, ""),
...
$(function() {
	if (urlParam.id) {
		if (!hasLoaded) {
			var a = $(".bills").hide();
			if (urlParam.turn) Public.ajaxGet(defaultPage.WDURL  + "/scm/invPo/queryDetails?action=queryDetails", {
				id: urlParam.id,
				from:'PU'
			}, function(b) {
				...
				if (!c.length) return;
				Public.ajaxGet(defaultPage.WDURL + "/basedata/inventory?action=list", {
					ids: c.join()
				}, function(a) {
					if (200 === a.status) {
						var c = a.data.rows || {};
						...
						for (var e = 0; e < b.length; e++) {
							...
							if ("object" == typeof f) {
								...
								if (f) {
									...
									var a = $.extend(!0, {}, f);
									if (a.goods = i, a.id = j, a.preQty = a.preQty || 1, a.qty = a.qty || 0, d) var k = $("#grid").jqGrid("setRowData", Number(d), {});
									else {
