function init() {
	void 0 !== cRowId ? Public.ajaxPost(Public.WDURL +"/basedata/contact/query?type=10&action=query", {
		id: cRowId
	}, function(a) {
	...
	$_form.validator({
		rules: {
			...
			unique: function(a, b, c) {
				var d = $(a).val();
				return $.ajax({
					url: Public.WDURL + "/basedata/contact/checkName?action=checkName",
					type: "get",
					data: "name=" + d,
					dataType: "json",
					success: function(a) {
			...
			myRemote: function(a, b, c) {
				return c.old.value === a.value || $(a).data("tip") === !1 && a.value.length > 1 ? !0 : $.ajax({
					url: Public.WDURL + "/basedata/contact/getNextNo?action=getNextNo&type=10",
					type: "post",
					data: "skey=" + a.value,
					dataType: "json",
					success: function(b) {
		...
		valid: function(a) {
			var b = $.trim($("#name").val());
			Public.ajaxPost(Public.WDURL +"/basedata/contact/checkName?type=10&action=checkName", {
				name: b,
				id: cRowId
			}, function(a) {
				-1 == a.status ? parent.$.dialog.confirm('供应商名称 "' + b + '" 已经存在！是否继续？', function() {
					postData()
				}, function() {}) : postData()
			})
		},
...
function postData() {
	var a = "add" == oper ? "新增供应商" : "修改供应商",
		b = getData(),
		c = b.firstLink || {};
	delete b.firstLink, Public.ajaxPost(Public.WDURL + "/basedata/contact/" + ("add" == oper ? "add" : "update")+"?type=10", b, function(d) {
		if (200 == d.status) {
...
function initEvent() {
	...
	b.blur(function() {
		...
		parent.$.dialog({
			title: "联系地址",
			content: "url:/views/settings/addressManage",
			data: {
				rowData: d,
...
function addCategory() {
	Business.verifyRight("SUPPLYTYPE_ADD") && parent.$.dialog({
		title: "新增供应商类别",
		content: "url:/views/settings/vendor_category_manage",
		data: {
			oper: "add",
			callback: function(a, b, c) {
				categoryCombo.loadData(Public.WDURL + "/basedata/assist?action=list&typeNumber=supplytype", ["id", a.id]), c && c.api.close()
			}
		},
		width: 400,
		height: 100,
