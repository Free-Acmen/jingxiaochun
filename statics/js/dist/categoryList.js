	...
	$("#grid").jqGrid({
		...
		loadComplete: function(a) {
			if (a && 200 == a.status) {
				...
				showCode = ("raccttype" === conditions.typeNumber||"paccttype" === conditions.typeNumber) ? true : false;
				if(showCode){
			
					jQuery("#grid").setGridParam().showCol("categoryCode");
					jQuery("#grid").setGridParam().showCol("categoryTitle");
					jQuery("#grid").setGridParam().showCol("categoryCodeTar");
					jQuery("#grid").setGridParam().showCol("categoryTitleTar");
				}
				else{
							//隐藏两列
					jQuery("#grid").setGridParam().hideCol("categoryCode");
					jQuery("#grid").setGridParam().hideCol("categoryTitle");
					jQuery("#grid").setGridParam().hideCol("categoryCodeTar");
					jQuery("#grid").setGridParam().hideCol("categoryTitleTar");
...
function postData(a) {
	...
	var f = {
		...
		categoryCodeTar:$("#categoryCodeTar").val(),
		categoryTitleTar:$("#categoryTitleTar").val(),
...
var conditions = {
	...
	handle = {
		operate: function(a, b) {
			...
			'<div class="label-wrap"><label for="ParentCategory">上级分类:</label></div>',
			...
			'<div class="dn hideFeild"></div>', 
			"</li>", 
			'<li class="row-item">',
			'<div class="label-wrap"><label for="category">类别:</label></div>', 
			'<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="category" id="category"></div>', 
			"</li>",
			
			'<li class="row-item" style="position:relative; display:none;">',
			'<div class="label-wrap"><label for="categoryTitle">借方科目:</label></div>', 
			'<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="categoryTitle" id="categoryTitle"></div>', 
			"</li>",
			
			'<li class="row-item" style="position:relative; display:none;">',
			'<div class="label-wrap"><label for="categoryCode">借方代码:</label></div>', 
			'<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="categoryCode" id="categoryCode"></div>', 
			"</li>",
					 
			'<li class="row-item" style="position:relative; display:none;">',
			'<div class="label-wrap"><label for="categoryTitleTar">贷方科目:</label></div>', 
			'<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="categoryTitleTar" id="categoryTitleTar"></div>', 
			"</li>",
			
			'<li class="row-item" style="position:relative; display:none;">',
			'<div class="label-wrap"><label for="categoryCodeTar">贷方代码:</label></div>', 
			'<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="categoryCodeTar" id="categoryCodeTar"></div>', 
			"</li>",
			...
			showParentCategory && (e = 150), this.dialog = $.dialog({
				...
				init: function() {
					...
					if (showCode)
					{
						$("#categoryCode").closest("li").show();
						$("#categoryTitle").closest("li").show();
						$("#categoryCodeTar").closest("li").show();
						$("#categoryTitleTar").closest("li").show();
					}
					else{
						$("#categoryCodeTar").closest("li").hide();
						$("#categoryTitleTar").closest("li").hide();
						$("#categoryCode").closest("li").hide();
						$("#categoryTitle").closest("li").hide();
					}
					...
					if (showParentCategory && (d.closest("li").show(), $("#ParentCategory").click(function() {
						...
						$("#categoryCode").val(f.categoryCode);
						$("#categoryTitle").val(f.categoryTitle);
						$("#categoryCodeTar").val(f.categoryCodeTar);
						$("#categoryTitleTar").val(f.categoryTitleTar);
