...
function postCustomerData() {
	var a = "add" == oper ? "新增客户" : "修改客户",
		b = getCustomerData(),
		c = b.firstLink || {};
	delete b.firstLink, Public.ajaxPost("../basedata/contact/" + ("add" == oper ? "addx" : "updatex"), b, function(d) {
		if (200 == d.status) {
			if (parent.parent.Public.tips({
				content: a + "成功！"
			}), callback && "function" == typeof callback) {
				var e = d.data.id;
				d = b, d.id = e, d.customerType = d.cCategoryName, d.contacter = c.linkName || "", d.place = c.linkPlace || "", d.mobile = c.linkMobile || "", d.telephone = c.linkPhone || "", d.linkIm = c.linkIm || "", d.deliveryAddress = c.address || "", d.province = c.province || "", d.city = c.city || "", d.ywy = c.ywy || "",d.county = c.county || "", callback(d, oper, window)
			}
...
//			ywy :salesCombo.getText(),
//			ywyid:salesCombo.getValue(),
//			
...
function initEvent() {
	...
	salesCombo = Business.salesCombo($("#staff"), {
		editable: !0,
		trigger: !0,
		width: 210,
	} );
...
var curRow, curCol, curArrears, api = frameElement.api,
	...
	categoryCombo, salesCombo,levelCombo, categoryData = {},
	$grid = $("#grid"),
	$_form = $("#manage-form");
initPopBtns(), init();