...
function addUrlParam() {
  ...
  $("#nav").find("li.item-report .nav-item a").each(function() {
      ...
      this.href = "商品库存余额表" === $(this).html() ? b + "beginDate=" + SYSTEM.startDate + "&endDate=" + SYSTEM.endDate : b + a
  })
}
function BBSPop() {
  var a = $("#yswb-tab")
...
var dataReflush, list = {
  ...
  purchase: {
      name: "购货单",
      href: "./scm/invPu/initPur.html?action=initPur",
      dataRight: "PU_ADD",
      target: "purchase",
      list: "./scm/invPu/initPurList.html?action=initPurList"
  },
  purchaseBack: {
      name: "购货退货单",
      href: "./scm/invPu/initPur.html?action=initPur&transType=150502",
      dataRight: "PU_ADD",
      target: "purchase",
      list: "./scm/invPu/initPurList.html?action=initPurList&transType=150502"
  },
  salesOrder: {
      name: "客户订单",
      href: "./scm/invSo/initSo.html?action=initSo",
      dataRight: "SO_ADD",
      target: "sales",
      list: "./scm/invSo/initSoList.html?action=initSoList"
  },
  sales: {
      name: "销货单",
      href: "./scm/invSa/initSale.html?action=initSale",
      dataRight: "SA_ADD",
      target: "sales",
      list: "./scm/invSa/initSaleList.html?action=initSaleList"
  },
  salesBack: {
      name: "销货退货单",
      href: "./scm/invSa/initSale.html?action=initSale&transType=150602",
      dataRight: "SA_ADD",
      target: "sales",
      list: "./scm/invSa/initSaleList.html?action=initSaleList&transType=150602"
  },
  transfers: {
      name: "调拨单",
      href: "./scm/invTf/initTf.html?action=initTf",
      dataRight: "TF_ADD",
      target: "storage",
      list: "./scm/invTf/initTfList.html?action=initTfList"
  },
  inventory: {
      name: "库存查询",
      href: "./storage/inventory.html",
      dataRight: "PD_GENPD",
      target: "storage"
  },
  otherWarehouse: {
      name: "其他入库单",
      href: "./scm/invOi/initOi-in.html?action=initOi&type=in",
      dataRight: "IO_ADD",
      target: "storage",
      list: "./scm/invOi/initOiList-in.html?action=initOiList&type=in"
  },
  otherOutbound: {
      name: "其他出库单",
      href: "./scm/invOi/initOi-out.html?action=initOi&type=out",
      dataRight: "OO_ADD",
      target: "storage",
      list: "./scm/invOi/initOiList-out.html?action=initOiList&type=out"
  },
  /*adjustment: {
  name: "成本调整单",
  ...
  receipt: {
      name: "收款单",
      href: "./scm/receipt/initReceipt.html?action=initReceipt",
      dataRight: "RECEIPT_ADD",
      target: "money",
      list: "./scm/receipt/initReceiptList.html?action=initReceiptList"
  },
  payment: {
      name: "付款单",
      href: "./scm/payment/initPay.html?action=initPay",
      dataRight: "PAYMENT_ADD",
      target: "money",
      list: "./scm/payment/initPayList.html?action=initPayList"
  },
  //verification: {
  //		name: "核销单",
  ...
  otherIncome: {
      name: "其他收入单",
      href: "./scm/ori/initInc.html?action=initInc",
      dataRight: "QTSR_ADD",
      target: "money",
      list: "./scm/ori/initIncList.html?action=initIncList"
  },
  otherExpense: {
      name: "其他支出单",
      href: "./scm/ori/initExp.html?action=initExp",
      dataRight: "QTZC_ADD",
      target: "money",
      list: "./scm/ori/initExpList.html?action=initExpList"
  },
...
serialnoList: {
  ...
  assistingProp: {
      name: "辅助属性",
      href: "./settings/assistingprop.html",
  ...
  _display: function(a, b) {
      for (var c = a.length - 1; c >= 0; c--){
        this.sublist[a[c]] && (this.sublist[a[c]].disable = !b);
      }
      return this
  },
  _show: function(a) {
  ...
  _menuControl: function() {
      ...
      b && (3 == c && this._show(["reInitial"]),
      this._show(["authority"]),
      this._show(["backup"])),
      2 == a && (1 == SYSTEM.hasOnlineStore ? this._show(["onlineStoreMap", "onlineStoreList", "onlineStoreRelation", "logisticsList", "orderHandle1", "orderHandle2", "orderHandle3", "orderList", "onlineSalesList"]) : 1 == SYSTEM.enableStorage && $(".vip-nav").width(125),
      1 == SYSTEM.enableStorage ? this._show(["JDStorageList", "JDStorageGoodsList", "JDStoragePurchaseOrderList", "JDStorageSaleOrderList", "JDStorageInvManage"]) : 1 == SYSTEM.hasOnlineStore && $(".vip-nav").width(120),
      1 == SYSTEM.enableAssistingProp && this._show(["assistingProp"])),
      ...
      if (this.sublist && this.obj) {
          ...
          for (var c in a)
              if (!a[c].disable) {
                  ...
                  if (d.name == "销售利润表") {
                      ...
                      e ? e.append(j) : (b.target[d.target] = this.objCopy.find("#" + d.target),
                      b.target[d.target] && b.target[d.target].append(j))
                  } else {
          ...
          this.objCopy.find("li.item").each(function() {
              ...
              a.find(".nav-item").each(function() {
                  var a = $(this);
                  a.find("li").length || (a.hasClass("last") && a.prev().addClass("last"),
                  a.remove())
              })
          }),
          this._setDom()
...
function(a) {
  if (2 === SYSTEM.siVersion && SYSTEM.isOpen) {
      var b, c = location.protocol + "//" + location.host + "/update_info.jsp", d = '您的单据分录已经录入达到300条，继续使用选择<a href="http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes" target="_blank">购买产品</a>或者完善个人信息赠送1000条免费容量。';
      SYSTEM.isshortUser ? SYSTEM.isshortUser && (b = "http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes&updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d) + "&loginPage=http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes&",
      a.dialog({
          min: !1,
          max: !1,
          cancle: !1,
          ...
          title: "完善个人信息",
          content: "url:" + b
      })) : (b = "http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes&updateUrl=" + encodeURIComponent(c) + "&warning=" + encodeURIComponent(d),
      a.dialog({
          min: !1,
          max: !1,
          cancle: !1,
...
$(window).load(function() {
  ...
  if ($("#scollUp").click(function() {
      ...
      a.first().prev().length > 0 && (a.first().prev().show(500),
      a.last().hide())
  }),
  $("#scollDown").click(function() {
      var a = b.filter(":visible");
      a.last().next().length > 0 && (a.first().hide(),
      a.last().next().show(500))
  }),
  ...
  $("#nav").on("click", "#reInitial", function(a) {
      a.preventDefault(),
      $.dialog({
          lock: !0,
          width: 430,
          ...
          ok: function() {
              if ($("#understand").is(":checked")) {
                  ...
                  $.ajax({
                      ...
                      dataType: "json",
                      success: function(b) {
                          200 === b.status && ($("#container").html(""),
                          a.close(),
                          window.location.href = "../home/index?re-initial=true&serviceType=" + SYSTEM.serviceType)
                      },
                      error: function(a) {
                          Public.tips({
                              type: 1,
