	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			if (this.salesCombo = Business.billSalesCombo($("#sales"), {
				...
				callback: {
					onChange: function(a) {
						//add by michen 20170724 begin
						if(a){
						var linkMen=$.parseJSON(a.linkMen);//json字符串直接转对象
				...
				setTimeout(function(){THISPAGE.linkCombo.selectByValue(a.udf01)},600);
				//add by michen 20170724 end
				for (var e = 0; e < SYSTEM.salesInfo.length; e++) if (SYSTEM.salesInfo[e].id === a.salesId) {
					...
					if ("" == b.customerCombo.input.val()) {
						...
						if(linkMan){
							b.$linkMan.find("input").val(linkMan.linkName);
							b.$linkPhone.find("input").val(linkMan.linkMobile?linkMan.linkMobile:linkMan.linkPhone);
							b.$linkAddress.find("input").val((linkMan.province?linkMan.province:'')+(linkMan.city?linkMan.city:'')+(linkMan.county?linkMan.county:'')+linkMan.address);
						}
						THISPAGE.linkCombo = Business.linkCombo(THISPAGE);
						setTimeout(function(){linkMan && THISPAGE.linkCombo.selectByValue(linkMan.linkName)},600);
						//add by michen 20170724 end
					}
			});
			...
			var f = '<!--<a id="original" class="ui-btn ui-btn-sc">选择源单</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">保存并新增</a><a id="save" class="ui-btn">保存</a>',
				...
				j = "",
				k = "";
				sfa= "150601" == a.transType?'<a class="ui-btn" id="sure">保存出库</a>':'<a class="ui-btn" id="sure">保存入库</a>';
			 	sfb= "150601" == a.transType?'<a class="ui-btn" id="sureall">完成出库</a>':'<a class="ui-btn" id="sureall">完成入库</a>';
			debugger
			if(a.surealled==1){
				sfa="";
			...
			if (f += k, this.btn_edit = g, this.btn_audit = i, this.btn_view = h, this.btn_reaudit = j, this.btn_tempSave = k, a.id > 0) {
				...
				} else a.checked ? ($("#mark").addClass("has-audit"), this.$_toolBottom.html('<span id="groupBtn">' + h + j +sfa+ "</span>" + l)) : this.$_toolBottom.html('<span id="groupBtn">' + h + "</span>" + l);
			...
			} else billRequiredCheck ? this.$_toolBottom.html("<span id=groupBtn>" + f + i +sfa+ "</span>") : this.$_toolBottom.html('<span id="groupBtn">' + f + "</span>"), this.$_userName.html(system.realName || ""), this.$_modifyTime.parent().hide(), this.$_createTime.parent().hide(), this.$_checkName.parent().hide();
			...
			function v(a) {
				...
				if (b) {
					if (b.batch || $("#grid").jqGrid("setCell", a, "batch", "&#160;"), b.safeDays || ($("#grid").jqGrid("setCell", a, "prodDate", "&#160;"), $("#grid").jqGrid("setCell", a, "safeDays", "&#160;"), $("#grid").jqGrid("setCell", a, "validDate", "&#160;")), 1 == b.isWarranty && $("#grid").jqGrid("showCol", "batch"), b.safeDays > 0 && ($("#grid").jqGrid("showCol", "prodDate"), $("#grid").jqGrid("showCol", "safeDays"), $("#grid").jqGrid("showCol", "validDate")), !b.price) {
						...
						if (c && c.id) {
							var d = b.salePrice,
//								e = [b.salePrice, b.wholesalePrice, b.vipPrice, b.discountRate1, b.discountRate2];//add by michen 20171208 for 客户等级价格
//							d = c.cLevel < 3 ? e[c.cLevel] : (1e4 * b.salePrice * e[c.cLevel] / 1e6).toFixed(2), b.price = d
								
									e = [b.salePrice, b.wholesalePrice, b.vipPrice, b.discountRate1, b.discountRate2];//add by michen 20171208 for 客户等级价格
								////d = c.cLevel < 3 ? e[c.cLevel] : (1e4 * b.salePrice * e[c.cLevel] / 1e6).toFixed(2),b.price = d
								d = c.cLevel < 5 ? e[c.cLevel] : (1e4 * b.salePrice * e[c.cLevel] / 1e6).toFixed(2),b.price = d
								if(c.cLevel>4){
									var ret=getPrice(b.number,c.id);
									if(ret.success){
										b.price =ret.data;
									}
									else{
					...
					if (SYSTEM.ISSERNUM && 1 == b.isSerNum && (f.preQty = f.serNumList ? f.serNumList.length : 0), f.preQty > 0) {
						var g = parseFloat(f.preQty),
					...
					f.amount = f.amount ? f.amount : f.price * f.preQty;
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				if (b.preventDefault(), Business.verifyRight("SA_SURE")) {
					var c = $(this),
						d = THISPAGE.getPostData();
						d.sa_sure=1;
					    d.surealled=0;
				...
				if (b.preventDefault(), Business.verifyRight("SA_SUREALL")) {
					var c = $(this),
						d = THISPAGE.getPostData();
						d.sa_sure=1;
					   d.surealled=1;
