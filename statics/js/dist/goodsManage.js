...
function getCustomerData() {
	...
	if (a) {
		var b = {
			...
			name: $.trim($("#name").val()),
			udf03: $.trim($("#udf03").val()),
			udf04: $.trim($("#udf04").val()),
			udf05: $.trim($("#udf05").val()),
...
function initField() {
	$("#note").placeholder(), "edit" == oper ? ($("#udf04").val(rowData.udf04),$("#udf05").val(rowData.udf05),$("#udf03").val(rowData.udf03),$("#number").val(rowData.number), $("#name").val(rowData.name), $category.data("defItem", rowData.categoryId), $("#specs").val(rowData.spec), $("#storage").data("defItem", rowData.locationId), $("#unit").data("defItem", ["id", rowData.baseUnitId]), void 0 != rowData.purPrice && $("#purchasePrice").val(Public.numToCurrency(rowData.purPrice, pricePlaces)), void 0 != rowData.salePrice && ($("#salePrice").val(Public.numToCurrency(rowData.salePrice, pricePlaces)),
...
function initEvent() {
	...
	categoryTree = Public.categoryTree($category, b), $("#specs").blur(function() {
		...
		b.click(function(d) {
			...
			if (b.text()=="售价设定") {
				var gn=$("#number").val();
				var src="/priceset.html?gn="+gn+"&q="+new Date().getTime();
				$("#pagePriceFrame").attr("src",src);
			}
		})
	}), $(document).bind("click.cancel", function(a) {
		null !== curRow && null !== curCol && (!$(a.target).closest("#grid").length > 0 && !$(a.target).closest(".pika-single").length > 0 && $grid.jqGrid("saveCell", curRow, curCol),!$(a.target).closest("#gridCombination").length > 0 && $gridCombination.jqGrid("saveCell", curRow, curCol), !$(a.target).closest("#gridWarning").length > 0 && $gridWarning.jqGrid("saveCell", curRow, curCol), !$(a.target).closest("#gridSku").length > 0 && $gridSku.jqGrid("saveCell", curRow, curCol))
...
function resetForm(a) {
	var b = [{}, {}, {}, {}];
	$("#name").val(""), $("#specs").val(""), $("#purchasePrice").val(""), $("#salePrice").val(""),
		$("#wholesalePrice").val(""), 
		$("#vipPrice").val(""),
		$("#discountRate1").val(""),
		$("#discountRate2").val(""),
		$("#lowQty").val(""), $("#highQty").val(""), $("#note").val(""), $grid.jqGrid("clearGridData", !0).jqGrid("setGridParam", {
		data: b
