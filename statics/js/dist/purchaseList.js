...
var THISPAGE = {
	...
	loadGrid: function() {
		...
		$("#grid").jqGrid({
			url: defaultPage.WDURL + "/scm/invPu?action=list",
			...
			gridview: true,
			multiselect: true,
			colNames: ["操作", "单据日期", "单据编号", "供应商","商品","购货金额", "优惠后金额", "已" + c + "款", c + "款状态", "数量", "制单人", "审核人", "备注", "订单来源"],
			colModel: [{
				name: "operating",
				width: 60,
				fixed: !0,
	...
	addEvent: function() {
		...
		if (
			...
			"150502" == queryConditions.transType ? (e = "购货退货单", f = "purchase-purchaseBack", parent.cacheList.purchaseBackId = $("#grid").jqGrid("getDataIDs")) : parent.cacheList.purchaseId = $("#grid").jqGrid("getDataIDs"), parent.tab.addTabItem({
				tabid: f,
				text: e,
				url: defaultPage.WDURL + "../scm/invPu?action=editPur&id=" + b + "&flag=list" + d + "&transType=" + queryConditions.transType
			})
			}), 
			$(".grid-wrap").on("click", ".ui-icon-trash", function(a) {
				if (a.preventDefault(), Business.verifyRight("PU_DELETE")) {
					...
					$.dialog.confirm("您确定要删除该购货记录吗？", function() {
						Public.ajaxGet(defaultPage.WDURL  + "/scm/invPu/delete?action=delete", {
							id: b
						}, function(a) {
			...
			$(".wrapper").on("click", "#btn-batchDel", function(a) {
				...
				"150502" == queryConditions.transType && (d = "购货退货单"), $.dialog.confirm("您确定要删除选中的" + d + "吗？", function() {
					Public.ajaxPost(defaultPage.WDURL + "../scm/invPu/delete?action=delete", {
						id: c
					}, function(a) {
			...
			$(".wrapper").on("click", "#export", function(a) {
				...
				var f = defaultPage.WDURL + "../scm/invPu/exportInvPu?action=exportInvPu" + d;
				$(this).attr("href", f)
			}), billRequiredCheck
		) {
			{
			...
			$(".wrapper").on("click", "#audit", function(a) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void Public.ajaxPost(defaultPage.WDURL + "../scm/invPu/batchCheckInvPu?action=batchCheckInvPu", {
					id: c
				}, function(a) {
					if (200 === a.status) {
			...
			$(".wrapper").on("click", "#reAudit", function(a) {
				...
				var b = $("#grid").jqGrid("getGridParam", "selarrrow"),
					c = b.join();
				return c ? void Public.ajaxPost(defaultPage.WDURL +"/scm/invPu/rsBatchCheckInvPu?action=rsBatchCheckInvPu", {
					id: c
				}, function(a) {
					if (200 === a.status) {
		...
		$("#add").click(function(a) {
				if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
					...
					parent.tab.addTabItem({
						tabid: c,
						text: b,
						url: defaultPage.WDURL + "../scm/invPu?action=initPur&transType=" + queryConditions.transType
					})
				}
		});
