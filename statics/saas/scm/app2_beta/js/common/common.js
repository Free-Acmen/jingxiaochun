		...
		callback: {
			onChange: function(data){
				...
				if(data){
					...
					page.$linkAddress.find("input").val((linkMan.province?linkMan.province:'')+(linkMan.city?linkMan.city:'')+(linkMan.county?linkMan.county:'')+linkMan.address);
				}else{
	...
	opts = $.extend(true, {
		...
		data: function(){
			if(defaultPage.SYSTEM.goodsInfo) {
				...
				return defaultPage.WDURL + 'S/basedata/inventory?action=list';
			}
		}
	}, opts);
	opts.callback = {
	...
	var opts = $.extend(true, {
		...
		callback: {
			...
			incrementalSearch: function(pos, callback){
				var _self = this;
				var query = $_obj.val()
				Public.ajaxGet(defaultPage.WDURL + '/basedata/inventory?action=list', { rows: 20, skey: query }, function(data){
